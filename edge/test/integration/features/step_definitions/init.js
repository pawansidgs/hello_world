...
var environment = config['hello_world'].env;
...
console.log('&hello_world api: [' +  domains[environment] + ', ' + config['hello_world'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hello_world'].app, config['hello_world'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hello_world'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
